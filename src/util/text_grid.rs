...
use simulation::iterators::{CoordIterator};
use util::{GridSize2D};
...
pub fn render_into_grid(options: TextGridOptions, renderer: &Box<dyn GridCellRenderer>, world: &World) -> String {
  let v_border_char = '|';
  let h_border_char = '-';
  ...
  for y_off in 0..size_y {
    ...
    for x in 0..size_x {
      ...
      let a = cell_str.split("\n").map(|x| -> String { x.to_string() }).collect::<Vec<_>>();
      lines.push(a);
    }
    ...
    for cell_line in 0..options.cell_height {
      ...
      for x in 0..size_x {
        let s = if cell_line < lines[x].len() { lines[x][cell_line].to_string() } else { "".to_string() };
...
pub fn multiline_trim(string: String) -> String {
  let lines = string.split("\n").map(|x| -> String { x.trim().to_string() }).collect::<Vec<_>>();
